...
const Announcements = () => {
    ...
    const scrollToForm = () => {
        if (window.lenis && formRef.current) {
            window.lenis.scrollTo(formRef.current, { offset: -100 });
        } else {
    ...
    return (
        <div className="min-h-screen pt-32 pb-12 px-4 sm:px-6 lg:px-8 bg-primary relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                {/* Why Join & CTA */}
                <section ref={formRef} className="grid lg:grid-cols-2 gap-12 items-center mb-16">
                    <motion.div
                    ...
                    <AnimatePresence>
                        {showFloatBtn && (
                            <motion.button
                                ...
                                className="fixed bottom-24 right-4 md:bottom-8 md:right-8 z-[9999] px-6 py-3 rounded-full bg-gradient-to-r from-secondary to-accent text-white font-bold shadow-lg shadow-secondary/25 hover:shadow-xl hover:shadow-secondary/40 transition-all duration-300 flex items-center gap-2"
                            >
